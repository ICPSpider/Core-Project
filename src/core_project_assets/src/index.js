...
async function mint_nft() {
  ...
  const mintId = await core_project.mint_principal(name, principal);
  ...
  document.getElementById("nft_minted").src = await core_project.tokenURI(mintId);
...
async function onButtonPress(el) {
  ...
  const balanceOf = await core_project.balanceOf(principal_string);
  const name = await core_project.name();
  
  document.getElementById("balanceOf").innerText = "You are currenlty owning: " + balanceOf + " of " + name;// + balanceOf;
  const test = await core_project.galleryOf(principal_string);                                             
